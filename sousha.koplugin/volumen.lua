...
local HorizontalGroup = require("ui/widget/horizontalgroup") -- Cambiado de Horizontal a Vertical
...
function CloseMe:init()
    -- Obtener Volumen
    local volumen_actual = ApiCalls:GetVol()
    ...
    local volumen_act = TextBoxWidget:new{
        text = _("Volumen Actual: "..tostring(volumen_actual).."%"),
        face = self.face,
        width = window_width - (self.padding * 2),
        alignment = "center",
    }
    local function actualizar_texto_volumen(nuevo_vol)
        volumen_act.text = _("Volumen Actual: " .. tostring(nuevo_vol) .. "%")
        -- MAGIA AQUÍ: Solicitamos redibujar SOLO el área del texto del volumen
        volumen_act:init()
        UIManager:setDirty(self, function()
            return "ui", volumen_act.dimen
        end)
    end
    local botones = HorizontalGroup:new{
        align = "center",
        Button:new{
            text=_("Subir"),
            callback = function ()
                ApiCalls:SetVolume("plus")
                local new_val = ApiCalls:GetVol()
                actualizar_texto_volumen(new_val)
    ...
    local content_group = VerticalGroup:new{
        ...
        volumen_act,
        VerticalSpan:new{ width = Size.span.vertical_large },
        botones,-- Espacio entre texto y botón
        close_button,
    }
